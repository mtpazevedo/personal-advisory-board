...
function buildChatSystemPrompt(advisor, userProfile) {
  const profileBlock = buildUserProfileBlock(userProfile);
  const rhetoricBlock = advisor.rhetoric
    ? `\n\n--- HOW YOU ARGUE ---\n${advisor.rhetoric}`
    : '';
...
- Use your web search tool only when the conversation turns on current facts.${profileBlock}`;
}
